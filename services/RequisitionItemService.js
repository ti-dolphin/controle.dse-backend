...
class RequisitionItemService {
  ...
  async updateQuoteItemsSelected(id_requisicao, quoteItemsSelectedMap) {
    ...
    const quoteItems  = await this.calculateItemsTotal(updatedItems, id_requisicao);
    const updatedRequisition = await this.calculateShippingTotal(quoteItems, id_requisicao);
    const custo_total = Number(updatedRequisition.custo_total_itens) + Number(updatedRequisition.custo_total_frete);
  ...
  async calculateShippingTotal(quoteItems, id_requisicao) {
    ...
    if (!quoteItems.length > 0) {
      ...
      const updatedRequisition =await RequisitionRepository.update(Number(id_requisicao), { custo_total_frete: 0});
      return updatedRequisition;
    }
    const quoteIdList = quoteItems.map((item) => Number(item.id_cotacao));
    const quotesInQuoteItemList = await QuoteRepository.getMany({id_cotacao: { in: quoteIdList }});
    const shippingTotal = quotesInQuoteItemList.map((quote) => Number(quote.valor_frete) || 0).reduce((acc, curr) => acc + Number(curr || 0));
    const updatedRequisition = await RequisitionRepository.update(Number(id_requisicao),{ custo_total_frete: shippingTotal });
    return updatedRequisition;
  }
